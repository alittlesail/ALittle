...
void computeLP(word16_t signal[], word16_t LPCoefficientsQ12[])
{
	...
	if (acc64>MAXINT32) {
		...
		autoCorrelationCoefficient[0] = (word32_t)acc64;
	} else {
	...
	if (rightShiftToNormalise>0) { /* acc64 was not fitting on 32 bits so compute the other sum on 64 bits too */
		for (i=1; i<NB_LSP_COEFF+1; i++) {
			...
			/* normalise it */
			autoCorrelationCoefficient[i] = (word32_t)(SHR(acc64 ,rightShiftToNormalise));
		}
	...
	/*                                                                               */
	/*  iterations i = 2..10                                                         */
	/*       sum = r[i] + E a[j]*r[i-j] with j = 1..i-1 (a[0] is always 1)           */
	/*       a[i] = -sum/E                                                           */
	/*       iterations j = 1..i-1                                                   */
