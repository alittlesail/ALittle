...
public:
    ...
    // ÎÄ×Ö±àÂë×ª»»
	static std::string UTF82ANSI(const char* content);
	static std::string ANSI2UTF8(const char* content);
...
#ifdef _WIN32
#define SUTF8(text) ALittle::StringHelper::ANSI2UTF8(text)
#else
#define SUTF8(text) text
#endif

#define X2S(x) std::to_string(x)
#define S2I(x) std::atoi(x)
