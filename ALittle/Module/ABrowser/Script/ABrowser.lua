...
function ABrowserMain:HandleAddressLoad(event)
	...
	if ALittle.List_MaxN(url_list) == 1 then
		self:LoadModule("Module/" .. url_list[1] .. "/", url_list[1])
		return
	end
	...
	if ALittle.List_MaxN(url_list) == 2 then
		...
		if address_list[1] ~= nil then
			port = ALittle.Math_ToInt(address_list[2])
		end
	end
...
local ___COROUTINE = coroutine.running()
	...
	if ip ~= nil and self._debug ~= "debug" then
		...
		local result = A_ModuleSystem:LoadModule(module_base_path, module_name)
