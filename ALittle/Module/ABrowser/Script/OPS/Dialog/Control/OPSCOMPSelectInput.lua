...
OPSCOMPSelectInput = Lua.Class(ABrowser.OPSCOMPBase, "ABrowser.OPSCOMPSelectInput")
...
function OPSCOMPSelectInput:Ctor(ctrl_sys, center, field_config, data)
	...
	self._choose_btn.y_type = 3
	self._choose_btn:AddEventListener(___all_struct[-449066808], self, self.HandleChooseButtonClick)
	self:UpdateConfig()
end
...
function OPSCOMPSelectInput:HandleChooseButtonClick(event)
	...
	if dialog ~= nil then
		dialog:RemoveEventListener(___all_struct[-1600012379], self, self.HandleSelectCallback)
		dialog:AddEventListener(___all_struct[-1600012379], self, self.HandleSelectCallback)
	end
end
