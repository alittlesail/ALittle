...
OPSBaseDialog = Lua.Class(ALittle.DisplayLayout, "ABrowser.OPSBaseDialog")

function OPSBaseDialog:Ctor(ctrl_sys)
...
function OPSBaseDialog:HandleConfirmButtonClick(event)
	if self._batch then
		...
		for k, choose in ___ipairs(choose_list) do
			local param = {}
			if self._dialog_config.send_param ~= nil then
			...
			self:HandleRefrshResult(error, content)
		end
	else
...
OPSBaseDialog.HandleConfirmButtonClick = Lua.CoWrap(OPSBaseDialog.HandleConfirmButtonClick)

function OPSBaseDialog:HandleRefrshResult(error, content)
...
OPSBaseDialogManager = Lua.Class(nil, "ABrowser.OPSBaseDialogManager")
