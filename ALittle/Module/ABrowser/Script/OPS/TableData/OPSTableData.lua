...
OPSTableData = Lua.Class(ALittle.DisplayLayout, "ABrowser.OPSTableData")
...
function OPSTableData:Ctor(ctrl_sys, center, table_data_config, select_mode)
	...
	self._grid3.x_type = 3
	self._grid3.y_type = 3
	self._grid3.width_type = 4
	self._grid3.width_value = 5
	self._grid3.height_type = 4
	self._grid3.height_value = 5
	self._grid3.type = 2
	___rawset(self, "_search", OPSTableDataSearch(g_Control, self._center, self._table_data_config, self))
	self._search.valign = 1
	self._search.line_spacing = 2
	self._search.column_spacing = 5
	self._search:AddEventListener(___all_struct[-431205740], self, self.HandleSearchGroupResize)
	...
	___rawset(self, "_static_object", OPSTableDataHead(g_Control, self._center, self._table_data_config, self))
	self._screen.static_object_h = self._static_object
	___rawset(self, "_page_tail", OPSTableDataTail(g_Control, self._center, self._table_data_config, self))
...
function OPSTableData:SetDataList(list)
	...
	self._data_object = OPSTableDataSetData(g_Control, self._center, self._table_data_config, self, set_param)
	self._data_object:AddEventListener(___all_struct[-1600012379], self, self.HandleDataClick)
