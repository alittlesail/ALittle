...
OPSDate = Lua.Class(ALittle.DisplayLayout, "ABrowser.OPSDate")
...
function OPSDate:Show(target, time_text, select_type, default_hour, default_min, default_sec)
	...
	self.x = x
	self.y = y
	local loop = ALittle.LoopFunction(Lua.Bind(A_LayerManager.ShowFromRight, A_LayerManager, self, nil), 1, 1, 1)
	loop:Start()
...
function OPSDate.StringToTime(text)
	local time = nil
	local start_index, end_index, year, mon, day, hour, min, sec = string.find(text, "^(%d%d%d%d)-(%d%d)-(%d%d) (%d%d).(%d%d).(%d%d)$", 1, false)
	if start_index == nil then
		start_index, end_index, year, mon, day = string.find(text, "^(%d%d%d%d)-(%d%d)-(%d%d)$", 1, false)
		if start_index ~= nil then
			hour = "0"
			min = "0"
			sec = "0"
...
function OPSDate:RefreshDay(day)
	...
	local i = 1
	while true do
		if not(i <= index - 1) then break end
		local day_container_x = "_day_container_" .. i
	...
	local i = 1
	while true do
		if not(i <= day_count) then break end
