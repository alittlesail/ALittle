...
ALittleIDE.DisplayObjectS = JavaScript.Class(undefined, {
		...
		if (image_path === "") {
			...
			if (grid9) {
				...
				if (display_info === undefined) {
					ALittleIDE.g_IDETool.ShowNotice("错误", "图片不存在:" + image_path);
					return;
				}
	...
	RemoverToNilShowSet : function(text, json_content, need_reset, revoke_bind) {
		...
		if (json_content === "") {
			...
			if (error1 === undefined) {
				...
				if (include !== undefined) {
					if (ALittleIDE.g_IDEProject.project.control_map[include] === undefined) {
						ALittleIDE.g_IDETool.ShowNotice("错误", "指定__include或__extends不存在");
						return;
					}
				}
				...
				ALittleIDE.g_IDETool.ShowNotice("错误", "输入show设置错误");
				return;
			}
		}
		this._tab_child.UpdateHandleQuadLayout(this._tree_logic);
	...
	TableDataSet : function(text, need_reset, revoke_bind) {
		...
		if (display_object.text === "") {
			...
			if (error === undefined) {
				...
				ALittleIDE.g_IDETool.ShowNotice("输入错误", "输入数据错误");
				display_object.text = "";
				return;
			}
		}
	...
	ImagePathSelectCallback : function(text, callback, revoke_bind, path) {
		...
		callback(this, e);
		let surface = ALittle.System_LoadSurface(ALittleIDE.g_IDEProject.project.base_path + "Texture/" + path);
