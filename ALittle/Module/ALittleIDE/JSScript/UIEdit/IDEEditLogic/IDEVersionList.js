...
ALittleIDE.IDEVersionList = JavaScript.Class(ALittle.DisplayLayout, {
	...
	HandleVersionListMenu : function(event) {
		if (this._version_menu === undefined) {
			this._version_menu = this._ctrl_sys.CreateControl("ide_version_menu", this);
		}
		A_LayerManager.ShowFromRight(this._version_menu);
		this._version_menu.x = A_UISystem.mouse_x;
		this._version_menu.y = A_UISystem.mouse_y;
		this._version_menu._user_data = event.target._user_data;
	},
	HandleVersionDelete : function(event) {
		A_LayerManager.HideFromRight(this._version_menu);
		let version_info = this._version_menu._user_data;
		if (version_info === undefined) {
			return;
		}
		let callback = this.VersionDeleteImpl.bind(this, version_info);
		ALittleIDE.g_IDETool.DeleteNotice("删除", "确定要删除" + ALittle.Time_GetCurDate(version_info.data.update_time) + "(版本时间)这个版本吗?", callback);
	},
	VersionDeleteImpl : async function(version_info) {
		let param = {};
		param.__account_id = ALittleIDE.g_IDELoginManager.account_id;
		param.__session_id = ALittleIDE.g_IDELoginManager.session_id;
		param.platform = this._platform;
		...
		if (error !== undefined) {
			ALittleIDE.g_IDETool.ShowNotice("提示", "删除失败:" + error);
			return;
		}
	...
	HandleVersionCloseListMenu : function(event) {
		if (this._version_close_menu === undefined) {
			this._version_close_menu = this._ctrl_sys.CreateControl("ide_version_close_menu", this);
		}
		A_LayerManager.ShowFromRight(this._version_close_menu);
		this._version_close_menu.x = A_UISystem.mouse_x;
		this._version_close_menu.y = A_UISystem.mouse_y;
		this._version_close_menu._user_data = event.target._user_data;
	},
	HandleVersionCloseDelete : function(event) {
		A_LayerManager.HideFromRight(this._version_close_menu);
		let version_info = this._version_close_menu._user_data;
		if (version_info === undefined) {
			return;
		}
		let callback = this.VersionCloseDeleteImpl.bind(this, version_info);
		ALittleIDE.g_IDETool.DeleteNotice("删除", "确定要删除" + version_info.data.close_version + "(" + version_info.data.submit_platform + ")这个拦截版本吗?", callback);
	},
	VersionCloseDeleteImpl : async function(version_info) {
		let param = {};
		param.__account_id = ALittleIDE.g_IDELoginManager.account_id;
		param.__session_id = ALittleIDE.g_IDELoginManager.session_id;
		param.platform = this._platform;
	...
	HandleAddVersionClose : function(event) {
		if (this._version_close_dialog === undefined) {
			...
			ALittleIDE.g_IDECenter.dialog_layer.AddChild(this._version_close_dialog);
		}
	...
	HandleAddVersionCloseClick : async function(event) {
		...
		if (error !== undefined) {
			ALittleIDE.g_IDETool.ShowNotice("提示", "添加失败:" + error);
			return;
		}
		this.HandleRefreshVersionList(undefined);
	},
	...
	GetConfig : function() {
		...
		if (this.CheckDateString(version_info.big_version) === false) {
			ALittleIDE.g_IDETool.ShowNotice("错误", "大版本格式不正确，请调整格式为：YYYY-mm-dd-HH-MM-SS");
			return undefined;
		}
		version_info.install_version = this._export_install_version.text;
		if (this.CheckDateString(version_info.install_version) === false) {
			ALittleIDE.g_IDETool.ShowNotice("错误", "安装包版本格式不正确，请调整格式为：YYYY-mm-dd-HH-MM-SS");
			return undefined;
		}
		version_info.version_number = this._export_version_number.text;
		if (this.CheckVersionString(version_info.version_number) === false) {
			ALittleIDE.g_IDETool.ShowNotice("错误", "软件版本号格式不正确，请调整格式为：数字.数字.数字");
			return undefined;
		}
		return version_info;
	},
	HandleGenBigVersionImpl : function() {
		this._export_big_version.text = ALittle.Time_GetCurDate();
	},
	HandleGenBigVersion : function(event) {
		let func = this.HandleGenBigVersionImpl.bind(this);
		ALittleIDE.g_IDETool.DeleteNotice("更新大版本号", "更新大版本号会引起强制更新，确定更新吗？", func);
	},
	HandleGenInstallVersionImpl : function() {
		this._export_install_version.text = ALittle.Time_GetCurDate();
	},
	HandleGenInstallVersion : function(event) {
		let func = this.HandleGenInstallVersionImpl.bind(this);
		ALittleIDE.g_IDETool.DeleteNotice("更新安装包版本号", "更新安装包版本号会引起重新安装，确定更新吗？", func);
	},
...
ALittleIDE.IDEVersionWindows = JavaScript.Class(ALittle.DisplayLayout, {
	...
	SaveConfigImpl : function() {
		...
		if (install_info.install_name === "") {
			ALittleIDE.g_IDETool.ShowNotice("错误", "安装包名不能为空");
			return false;
		}
	...
	HandleSaveConfig : function(event) {
		...
		ALittleIDE.g_IDETool.ShowNotice("提示", "配置保存成功");
	},
	HandleExport : async function(event) {
		...
		ALittleIDE.g_IDETool.ShowNotice("提示", "JavaScript版本不支持导出项目功能");
	},
...
ALittleIDE.IDEVersionAndroid = JavaScript.Class(ALittle.DisplayLayout, {
	...
	SaveConfigImpl : function() {
		...
		if (install_info.package_name === "") {
			ALittleIDE.g_IDETool.ShowNotice("错误", "Android包名不能为空");
			return false;
		}
		install_info.new_log = this._export_new_log.text;
		ALittleIDE.g_IDEProject.project.config.SetConfig(this.config_key, export_info);
...
ALittleIDE.IDEVersioniOS = JavaScript.Class(ALittle.DisplayLayout, {
	...
	SaveConfigImpl : function() {
		...
		if (install_info.package_name === "") {
			ALittleIDE.g_IDETool.ShowNotice("错误", "iOS包名不能为空");
			return false;
		}
		install_info.new_log = this._export_new_log.text;
		ALittleIDE.g_IDEProject.project.config.SetConfig(this.config_key, export_info);
