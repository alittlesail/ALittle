...
JavaScript.JMsgInterface = JavaScript.Class(ALittle.IMsgCommonNative, {
	...
	HandleNetSystemOpen : function(event) {
		...
		let func = window["__ALITTLEAPI_ConnectSucceed"];
		if (func === undefined) {
			return;
		}
		func(this);
	...
	HandleNetSystemClose : function(event) {
		...
		this._net_status = JavaScript.JConnectStatus.NET_IDLE;
		this._net_system = undefined;
		let func = window["__ALITTLEAPI_Disconnect"];
		if (func === undefined) {
			return;
		}
		func(this);
	...
	HandleNetSystemError : function(event) {
		...
		this._net_status = JavaScript.JConnectStatus.NET_IDLE;
		this._net_system = undefined;
		let func = window["__ALITTLEAPI_ConnectFailed"];
		if (func === undefined) {
			return;
		}
		func(this);
	...
	HandleNetSystemMessage : function(event) {
		...
		while (true) {
			...
			let func = window["__ALITTLEAPI_Message"];
			if (func === undefined) {
				return;
			}
			func(id, rpc_id, factory, this);
		}
