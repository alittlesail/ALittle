...
function IDEExport:RepairPNG(path)
	...
	for file_path, attr in ___pairs(file_map) do
		if ALittle.File_GetFileExtByPath(file_path) == "png" then
			local surface = ALittle.System_LoadSurface(file_path)
			if surface ~= nil then
...
function IDEExport:HandleSubmitVersionCancel(event)
...
function IDEExport:GenerateExe(package_info)
...
function IDEExport:GenerateApk(package_info)
	...
	local surface = ALittle.System_LoadSurface(install_png)
	if surface ~= nil then
		local surface_width = ALittle.System_GetSurfaceWidth(surface)
		local surface_height = ALittle.System_GetSurfaceHeight(surface)
		local new_surface = ALittle.System_CreateSurface(24, 24)
		local new_surface_width = ALittle.System_GetSurfaceWidth(new_surface)
		local new_surface_height = ALittle.System_GetSurfaceHeight(new_surface)
...
function IDEExport:GenerateIpa(package_info)
	...
	if surface ~= nil then
		local surface_width = ALittle.System_GetSurfaceWidth(surface)
		local surface_height = ALittle.System_GetSurfaceHeight(surface)
		local new_surface = ALittle.System_CreateSurface(40, 40)
		local new_surface_width = ALittle.System_GetSurfaceWidth(new_surface)
		local new_surface_height = ALittle.System_GetSurfaceHeight(new_surface)
		ALittle.System_CutBlitSurface(new_surface, surface, "0,0," .. new_surface_width .. "," .. new_surface_height, "0,0," .. surface_width .. "," .. surface_height)
