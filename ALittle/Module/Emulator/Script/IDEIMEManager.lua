...
IDEIMEManager = Lua.Class(nil, "Emulator.IDEIMEManager")

function IDEIMEManager:Ctor()
end

function IDEIMEManager:Setup()
...
function IDEIMEManager:Shutdown()
	if self._ime_dialog ~= nil then
...
function IDEIMEManager:HandleImeEditingCallback(open, text, start, list, target)
	if self._ime_dialog == nil then
		return
	end
