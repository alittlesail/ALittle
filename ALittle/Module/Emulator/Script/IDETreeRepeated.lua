...
function IDETreeRepeated:Ctor(ctrl_sys, field_name, rflct, msg, field)
	...
	if self._cpp_type == 7 then
		...
		while true do
			...
			self:AddChild(IDETreeRepeatedBool(g_Control, self, rflct, msg, field, index))
			index = index+(1)
		end
	elseif self._cpp_type == 1 then
		...
		while true do
			if not(index < value_count) then break end
			self:AddChild(IDETreeRepeatedValue(g_Control, self, rflct, msg, field, index))
			index = index+(1)
		end
	...
	elseif self._cpp_type == 8 then
		...
		while true do
			if not(index < value_count) then break end
			self:AddChild(IDETreeRepeatedEnum(g_Control, self, rflct, msg, field, index))
			index = index+(1)
		end
...
function IDETreeRepeated:AddLine(index)
	if self._cpp_type == 7 then
		...
		self:AddChild(IDETreeRepeatedBool(g_Control, self, self._rflct, self._msg, self._field, index), index + 1)
	elseif self._cpp_type == 1 then
	...
	elseif self._cpp_type == 8 then
		protobuf.reflection_insertrepeatedint32(self._rflct, self._msg, self._field, index, 0)
		self:AddChild(IDETreeRepeatedEnum(g_Control, self, self._rflct, self._msg, self._field, index), index + 1)
	elseif self._cpp_type == 9 then
