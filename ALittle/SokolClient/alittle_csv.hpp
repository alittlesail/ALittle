...
public:
	...
	bool Load(const char* file_path, bool only_assets)
	{
		...
		bool result = ReadFromCustomFile(file_path, (CarpCsv::read_file)CARP_RWread, file, 0);
		CARP_RWclose(file);
		return result;
	}
};
...
{
public:
	ALittleCsvFileWrap() : m_file(0) {}
	~ALittleCsvFileWrap() { if (m_file) delete m_file; }
...
public:
	...
	int GetColCount() const { if (m_file == nullptr) return 0; return (int)m_file->GetColCount(); }
	int GetRowCount() const { if (m_file == nullptr) return 0; return (int)m_file->GetRowCount(); }
	void Close() { if (m_file) delete m_file; m_file = nullptr; }

private:
	ALittleCsvFile* m_file;
};
...
public:
	...
	{
		ALittleCsvFile* file = new ALittleCsvFile();
