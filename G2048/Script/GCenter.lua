...
g_GConfig = nil
GCenter = ALittle.Class(nil, "G2048.GCenter")
...
function GCenter:Setup()
	g_GConfig = ALittle.ClientConfigSystem(g_ModuleBasePath .. "/User.cfg")
...
function GCenter:GenerateItem(delay_time)
	...
	local rand1 = math.floor(math.random(1, 100))
	local number = 2
	if rand1 > 50 then
		number = 4
	end
	local rand2 = math.floor(math.random(1, list_count))
...
function GCenter:TransAndRemoveItem(item, from_row, from_col, to_row, to_col)
	...
	local loop = ALittle.LoopTimer(ALittle.Bind(self.EraseItem, self, item), 200)
	self._loop_group[loop] = loop
	A_LoopSystem:AddUpdater(loop)
...
function GCenter:HandleDragEnd(event)
	...
	self._item_moved = false
	self._loop_delay = 0
	if math.abs(self._drag_total_x) > math.abs(self._drag_total_y) then
