...
ABnfGuessError ALittleScriptTranslationLua::GeneratePathsValue(std::shared_ptr<ALittleScriptPathsValueElement> paths_value, std::string& content)
{
    ...
    std::string error;
    if (!index->GetDeepFilePaths(paths_value->GetFile()->GetProject(), paths_value->GetProjectPath() + path, u8"", path_list, error)) return ABnfGuessError(paths_value, error);
...
ABnfGuessError ALittleScriptTranslationLua::GenerateReflectStructInfo(ABnfGuessPtr guess)
{
    ...
    else if (std::dynamic_pointer_cast<ALittleScriptGuessStruct>(guess))
    {
        ...
        if (guess_struct->struct_dec.lock() != nullptr && guess_struct->struct_dec.lock()->GetFullPath() == m_file_path)
            generate = true;
        // 如果不在同一个工程，那么就生成
        if (guess_struct->struct_dec.lock() != nullptr && guess_struct->struct_dec.lock()->GetProjectPath() != m_project_path)
            generate = true;
