...
static int pusherror(lua_State *L, const char *info)
{
        ...
        if (info==NULL)
			...
			lua_pushfstring(L, "%s: %s", info, error_text);
#else
		lua_pushnil(L);
		if (info==NULL)
...
static int lfs_lock_dir(lua_State *L) {
  ...
  if(!ln) {
	char error_text[256];
	strerror_s(error_text, sizeof(error_text), errno);
    lua_pushnil(L); lua_pushstring(L, error_text); return 2;
  }
  strcpy_s(ln, pathl + strlen(lockfile) + 1, path); strcat_s(ln, pathl + strlen(lockfile) + 1, lockfile);
  if((fd = CreateFile(ln, GENERIC_WRITE, 0, NULL, CREATE_NEW,
		...
		{
			char error_text[256];
			strerror_s(error_text, sizeof(error_text), en);
            lua_pushstring(L, error_text);
		}
        return 2;
  }
...
static int lfs_g_setmode (lua_State *L, FILE *f, int arg) {
  ...
  exit:
	...
	strerror_s(error_text, sizeof(error_text), en);
    lua_pushnil(L);
    lua_pushfstring(L, "%s", error_text);
#else
	lua_pushnil(L);
