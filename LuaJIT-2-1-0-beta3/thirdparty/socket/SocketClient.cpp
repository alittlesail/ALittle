...
void SocketClient::Connect(const std::string& ip, unsigned int port)
{
	// 检查是否正在连接
	...
	// 开始异步连接
	m_socket->async_connect(ep, std::bind(&SocketClient::HandleAsyncConnect, this->shared_from_this(), std::placeholders::_1));
}

void SocketClient::HandleAsyncConnect(const asio::error_code& ec)
{
	if (ec)
	{
		// 这个日志不打印，因为会出现太多，又不重要
		// ALITTLE_SYSTEM(u8"SocketClient 连接失败: " << SUTF8(asio::system_error(ec).what()) << ", ip:" << m_ip << ", port:" << m_port);
...
void SocketClient::ExecuteDisconnectCallback()
{
...
void SocketClient::HandleConnectFailed()
{
...
void SocketClient::HandleConnectSucceed()
{
	socket_event* event = socket_createevent(m__socket);
...
void SocketClient::HandleDisconnected()
{
	socket_event* event = socket_createevent(m__socket);
...
void SocketClient::ReadNumber(int size, int read_type)
{
	// 如果已经释放了就直接返回
	if (!m_socket) return;
	...
	asio::async_read(*m_socket, asio::buffer(m_simple_value, size)
					, std::bind(&SocketClient::HandleReadNumber, this->shared_from_this()
					, std::placeholders::_1, std::placeholders::_2, read_type));
}

void SocketClient::HandleReadNumber(const asio::error_code& ec, std::size_t actual_size, int type)
{
	if (ec)
	{
		// 释放内存
...
void SocketClient::ReadBinary(int size, int read_type)
{
	// 如果已经释放了就直接返回
	if (!m_socket) return;
	...
	asio::async_read(*m_socket, asio::buffer(m_binary_value, size)
		, std::bind(&SocketClient::HandleReadBinary, this->shared_from_this()
			, std::placeholders::_1, std::placeholders::_2, read_type));
}

void SocketClient::HandleReadBinary(const asio::error_code& ec, std::size_t actual_size, int type)
{
	if (ec)
	{
		// 释放内存
...
void SocketClient::NextSend()
{
	...
	asio::async_write(*m_socket, asio::buffer(info.memory, info.memory_size)
		, std::bind(&SocketClient::HandleSend, this->shared_from_this()
		, std::placeholders::_1, std::placeholders::_2, info.memory));
}

void SocketClient::HandleSend(const asio::error_code& ec, std::size_t bytes_transferred, void* memory)
{
	// 释放内存
