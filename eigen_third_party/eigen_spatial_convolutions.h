...
//#include "eigen_third_party/eigen3/unsupported/Eigen/CXX11/Tensor"
 ...
 public:
  ...
  TensorContractionInputMapper(
    ...
    if (internal::traits<ArgType>::Layout == ColMajor) {
      ...
      const int NumDims = static_cast<int>(tensor.impl().dimensions().size());
    ...
    if (internal::traits<ArgType>::Layout == ColMajor) {
      ...
      const int NumDims = static_cast<int>(tensor.impl().impl().dimensions().size());
...
EIGEN_DEVICE_FUNC
  ...
  switch (padding_type) {
    case PADDING_VALID:
      out_height = static_cast<TensorIndex>(numext::ceil((InputRows - kernelRowsEff + 1.f) /
                                static_cast<float>(row_stride)));
      out_width = static_cast<TensorIndex>(numext::ceil((InputCols - kernelColsEff + 1.f) /
                               static_cast<float>(col_stride)));
      break;
    case PADDING_SAME:
      out_height = static_cast<TensorIndex>(numext::ceil(InputRows / static_cast<float>(row_stride)));
      out_width = static_cast<TensorIndex>(numext::ceil(InputCols / static_cast<float>(col_stride)));
      break;
    default:
