...
{
	auto* project_c = static_cast<ABnfProject*>(project);
...
{
	delete static_cast<ABnfProject*>(file);
}
...
{
	return new ABnfFile(static_cast<ABnfProject*>(project), "", full_path, text, len, true);
}
...
void abnffile_settext(void* abnf_file, int version, const char* text, size_t len)
{
	...
	auto* project_c = static_cast<ABnfFile*>(abnf_file)->GetProject();
	project_c->Add(std::bind(&ABnfProject::UpdateText, project_c, static_cast<ABnfFile*>(abnf_file)->GetFullPath(), version, std::string(text, len)));
}
...
void abnffile_inserttext(void* abnf_file, int version, const char* text, size_t len, int it_line, int it_char)
{
	...
	auto* project_c = static_cast<ABnfFile*>(abnf_file)->GetProject();
	project_c->Add(std::bind(&ABnfProject::InsertText, project_c, static_cast<ABnfFile*>(abnf_file)->GetFullPath(), version, std::string(text, len), it_line, it_char));
}
...
void abnffile_deletetext(void* abnf_file, int version, int it_line_start, int it_char_start, int it_line_end, int it_char_end)
{
	...
	auto* project_c = static_cast<ABnfFile*>(abnf_file)->GetProject();
	project_c->Add(std::bind(&ABnfProject::DeleteText, project_c, static_cast<ABnfFile*>(abnf_file)->GetFullPath(), version, it_line_start, it_char_start, it_line_end, it_char_end));
}

void abnffile_querycolor(void* abnf_file, int query_id, int version, int line)
{
	auto* project_c = static_cast<ABnfFile*>(abnf_file)->GetProject();
	project_c->Add(std::bind(&ABnfProject::QueryColor, project_c, static_cast<ABnfFile*>(abnf_file)->GetFullPath(), query_id, version, line));
}

void abnffile_queryinfo(void* abnf_file, int query_id, int version, int it_line, int it_char)
{
	auto* project_c = static_cast<ABnfFile*>(abnf_file)->GetProject();
	project_c->Add(std::bind(&ABnfProject::QueryInfo, project_c, static_cast<ABnfFile*>(abnf_file)->GetFullPath(), query_id, version, it_line, it_char));
}

void abnffile_querygoto(void* abnf_file, int query_id, int version, int it_line, int it_char)
{
	auto* project_c = static_cast<ABnfFile*>(abnf_file)->GetProject();
	project_c->Add(std::bind(&ABnfProject::QueryGoto, project_c, static_cast<ABnfFile*>(abnf_file)->GetFullPath(), query_id, version, it_line, it_char));
}

void abnffile_querycomplete(void* abnf_file, int query_id, int version, int it_line, int it_char)
{
	auto* project_c = static_cast<ABnfFile*>(abnf_file)->GetProject();
	project_c->Add(std::bind(&ABnfProject::QueryComplete, project_c, static_cast<ABnfFile*>(abnf_file)->GetFullPath(), query_id, version, it_line, it_char));
}

void abnffile_queryparamlist(void* abnf_file, int query_id, int version, int it_line, int it_char)
{
	auto* project_c = static_cast<ABnfFile*>(abnf_file)->GetProject();
	project_c->Add(std::bind(&ABnfProject::QueryParamList, project_c, static_cast<ABnfFile*>(abnf_file)->GetFullPath(), query_id, version, it_line, it_char));
}
...
{
	auto* project_c = static_cast<ABnfFile*>(abnf_file)->GetProject();
	project_c->Add(std::bind(&ABnfProject::QueryError, project_c, static_cast<ABnfFile*>(abnf_file)->GetFullPath(), query_id, version, force != 0));
}
